...
document.getElementById("og").value = "hello world"//sampleSentences[Math.floor(Math.random() * sampleSentences.length)]//.toLowerCase()
changeSettings()
...
function encodeInput() {
	formattedArray = []
	encoded = encode(document.getElementById("og").value, brackets)
...
function encode(inputString, brackets) {
	...
	return {
		...
		bracketed: bitArrayBracketed.indexes.join(" ") + " - " + bitArray.IDs.join(" | ")
	}
...
function createBitArray(idArray, dataset) {
	...
	for (let i = 0; i < idArray.length; i++) {
		...
		indexBitArray.push(dec2bin(dataset.indexingScheme.indexOf(indexAssignment) + 1, dataset.indexAssignmentBitLength))
